...
for page in range(1,21):
    ...
    for i in range(len(tr)):
        ...
        if not '(오픈준비중)' in td[0].text and '서울' in td[1].text:
            mart_name = td[0].text
...
with open(f"/home/ubuntu/csvfile/mart_list_{current_date}.csv", mode='w', newline='', encoding='utf-8') as file:
    writer = csv.writer(file)
    writer.writerows(mart_list)

with open(f"/home/ubuntu/csvfile/mart_Link_{current_date}.csv", mode='w', newline='', encoding='utf-8') as file:
    writer = csv.writer(file)
